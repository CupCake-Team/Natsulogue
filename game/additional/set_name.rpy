...
label set_name_input:
    $ persistent.player = renpy.input("Как тебя зовут?", length=32)
    $ persistent.player = persistent.player.strip()
    if persistent.player.lower() in react_n:
        $ input_count_n += 1
    elif persistent.player.lower() == "yuri" or persistent.player.lower() == "юри":
        $ input_count_y += 1
    elif persistent.player.lower() in react_m:
        $ input_count_m += 1
    elif perisistent.player.lower() in react_s:
        $ input_count_s += 1
    $ input_count += 1
    return
...
label set_name:
    ...
    if persistent.player.lower() in react_n:
    ...
    elif persistent.player.lower() in react_names:
        # НАЦ НАЗЫВАЕТ КЛОУНОМ
        call set_name_joker
        ...
        # САЙОРИ
        if perisistent.player.lower() in react_s:
            if input_count_s == 1:
            ...
            if input_count_s == 2:
                ...
                jump set_name
        # ЮРИ
        if persistent.player.lower() == "yuri" or perisistent.player.lower() == "юри":
            if input_count_y == 1:
                n "..."
            ...
            if input_count_y == 2:
                ...
                jump set_name
        # МОНИКА
        if persistent.player.lower() in react_m:
            if input_count_m == 1:
                n r1a "Ха?"
